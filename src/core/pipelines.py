    ...
    def mouse_callback(event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONUP:
            ...
            for n in range(n_particles):
                ...
                particles.extend([Particle(kernel_size, crop_size, nn_size, p=3, q=1e-9, temperature=0.1)])
                particles[n].reset()
                particles[n].create_kernel(cv2.cvtColor(img, cv2.COLOR_BGR2GRAY), xy)
                ...
                if particles[0].coordinates is not None:
                    for i, particle in enumerate(particles):
                        ...
                        #     particles2duplicate.append([i])
                            ...
                            cv2.circle(img, xy, kernel_size // 2, (0, 255, 0), 2)
