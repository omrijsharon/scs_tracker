...
class SCS_Multi_Kernel_Tracker:
    def __init__(self, kernel_size, crop_size, nn_size=3, max_diffusion_radius=11, p=3, q=1e-9, temperature=0.05, max_velocity=50):
        ...
        # assert that all kernel sizes are odd and that they are all smaller than crop_size
        assert all(kernel_size % 2 == 1 for kernel_size in self.kernel_size), "all kernel sizes must be odd"
        assert all(kernel_size < crop_size for kernel_size in self.kernel_size), "all kernel sizes must be smaller than crop_size"
        self.is_majority_vote = True
    ...
    def create_kernel(self, frame, yx):
        ...
        for kernel_size in self.kernel_size:
            kernel, _, _ = crop_frame(frame, self.coordinates, kernel_size)
    ...
    def update(self, frame):
        ...
        try:
            ...
            max_idx = np.zeros((len(self.kernel), 2), dtype=np.int)
            max_change = np.zeros(len(self.kernel))
            for i, kernel in enumerate(self.kernel):
                filtered_scs_frame = calc_scs(cropped_frame, kernel, p=self.p, q=self.q)
                max_idx[i], max_change[i] = self.find_max(filtered_scs_frame)
            # find the kernel with the highest max_change
            max_kernel_idx = np.argmax(max_change)
            self.max_change = max_change[max_kernel_idx]
            if self.is_majority_vote:
                # majority vote for the index: choose the index that appears the most. if there is no majority, choose the index with the highest max_change
                # make max_idx a list of tuples:
                max_idx = [tuple(idx) for idx in max_idx]
                idx_count = Counter(max_idx)
                # Find the maximum count value
                max_count = max(idx_count.values())
                print("max_count:", max_count)
                # Find the tuples that have the maximum count
                most_common_indices = [k for k, v in idx_count.items() if v == max_count]
                # if there is a majority, choose the index with the highest max_change
                if len(most_common_indices) == 1:
                    idx = np.array(most_common_indices[0])
                else:
                    # if there is no majority, choose the index with the highest max_change
                    idx = np.array(max_idx[max_kernel_idx])
            else:
                idx = np.array(max_idx[max_kernel_idx])  # choose the index of the kernel with the highest max_change
        except:
    ...
    def draw_square_around_xy(self, frame, square_size=None):
        ...
        if self.is_successful:
            for sq_size in square_size:
                cv2.rectangle(frame, tuple((self.coordinates - sq_size // 2).astype(np.int)[::-1]), tuple((self.coordinates + sq_size // 2).astype(np.int)[::-1]), self.color, 2)
