...
class SIFT_tracker:
    def __init__(self, n_closest_kp):
        ...
        self.crop_scale = 70
        self.detector = cv.SIFT_create(nfeatures=1000, contrastThreshold=0.001, sigma=2.0, nOctaveLayers=3, edgeThreshold=10)
        matcher = cv.BFMatcher(cv.NORM_L2, crossCheck=True)  # This is for SIFT
    ...
    def reset(self, frame, xy): # gets the entire frame and not only a crop
        ...
        self.crop_size = np.array([71, 71])
        cropped_frame, self.top_left = crop_frame(frame, np.array(xy), self.crop_size)
        self.kp, des = self.detector.detectAndCompute(cropped_frame, None)
    ...
    def update(self, frame):
        ...
        if self.is_successful:
            ...
            self.crop_size = np.clip(self.crop_size, a_min=11, a_max=201)
            cropped_frame, self.top_left = crop_frame(frame, self.xy, self.crop_size)
            self.kp, des = self.detector.detectAndCompute(cropped_frame, None)
            self.kp = correct_kp_coordinates(self.kp, self.top_left)
        else:
            self.kp, des = self.detector.detectAndCompute(frame, None)
        if len(self.kp) > 0:
