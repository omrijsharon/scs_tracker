...
class OpticalFlowOdometery:
    def __init__(self,
        ...
        Raises:
            ValueError -- Raised when file either file paths are not correct, or img_file_path is not configured correctly
        '''
        self.cap = sc.ScreenCapture(monitor_number=1, tlwh=sc.YOUTUBE_TLWH_SMALL)
        self.detector = detector
        self.lk_params = lk_params
        self.focal = focal_length
        self.pp = pp
    ...
    def process_frame(self):
        ...
        if self.id < 2:
            self.old_frame = self.cap.capture()
            self.current_frame = self.cap.capture()
            self.visual_odometery()
            self.id = 2
        else:
            self.old_frame = self.current_frame
